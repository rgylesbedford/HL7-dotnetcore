...
namespace HL7.Dotnetcore.Test
{
    ...
    {
        private readonly string _hL7_ORM;
        private readonly string _hL7_ADT;

        public HL7Test()
        {
            var path = Path.GetDirectoryName(typeof(HL7Test).GetTypeInfo().Assembly.Location) + "/";
            _hL7_ORM = File.ReadAllText(path + "Sample-ORM.txt");
            _hL7_ADT = File.ReadAllText(path + "Sample-ADT.txt");
        }

        [TestMethod]
        public void SmokeTest()
        {
            var message = new Message(_hL7_ORM);
        ...
        {
            var message = new Message(_hL7_ADT);
        ...
        public void ReadSegmentTest()
        {
            ...
            var MSH_1 = message.Segments("MSH")[0];
        ...
        public void AddComponentsTest()
        {
            ...
            var newSeg = new Segment("ZIB", encoding);

            // Create Field ZIB_1
            var ZIB_1 = new Field("ZIB1", encoding);
            // Create Field ZIB_5
            var ZIB_5 = new Field("ZIB5", encoding);

            // Create Component ZIB.5.3
            var com1 = new Component("ZIB.5.3_", encoding);
            ...
            var serializedMessage = message.SerializeMessage(false);
        ...
        {
            var sampleMessage = "MSH|^~\\&|SA|SF|RA|RF|20110613083617||ADT^A04|123|P|2.7||||\r\nEVN|A04|20110613083617||\"\"\r\n";
        ...
        {
            var sampleMessage = "MSH|^~\\&|Sending\tApplication|Sending\tFacility|RA|RF|20110613083617||ADT^A04|123|P|2.7||||\r\nEVN|A04|20110613083617\r\n";
            var message = new Message(sampleMessage);
            message.ParseMessage();
            var serialized = message.SerializeMessage(false);
            Assert.AreEqual(sampleMessage, serialized);
        ...
        {
            var date = MessageHelper.ParseDateTime("20151231234500.1234-2358", out var offset).Value;
            // Assert.AreEqual(0, d
        ...
        public void ParseDateTimeOffset_WithException()
        {
            try
            {
                var date = MessageHelper.ParseDateTime("201", out var offset, true);
                Assert.Fail();
            }
            catch (AssertFailedException)
            {
...
PV1||O|NWSLED^^^NYULHLI^^^^^LI NW SLEEP DISORDER^^DEPID||||1447312459^DOE^MICHAEL^^^^^^EPIC^^^^PNPI~DOEM06^DOE^MICHAEL^^^^^^KID^^^^KID|1447312459^DOE^MICHAEL^^^^^^EPIC^^^^PNPI~DOEM06^DOE^MICHAEL^^^^^^KID^^^^KID|||||||||||496779945|||||||||||||||||||||||||20191107|||||||V";
            ...
            var attendingDrId = message.GetValue(index);
        ...
        public void RemoveTrailingComponentsTest_OnlyTrailingComponentsRemoved()
        {
            ...
            for (var eachField = 1; eachField <= 12; eachField++)
            {
                orcSegment.AddEmptyField();
            }

            for (var eachComponent = 1; eachComponent < 8; eachComponent++)
            {
